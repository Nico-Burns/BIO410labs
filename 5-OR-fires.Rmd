...
```{r}
terwilliger_rast <- rast("~/Documents/BIO410stuff/5-OR-fires/soil-burn-severity/2018_terwilliger_sbs/SoilSeverity.tif")
beachiecreek_rast <-rast("~/Documents/BIO410stuff/5-OR-fires/soil-burn-severity/2020_beachiecreek_sbs/BeachieCreek_SBS_final.tif")
holidayfarm_rast <- rast("~/Documents/BIO410stuff/5-OR-fires/soil-burn-severity/2020_holidayfarm_sbs/HolidayFarm_SBS_final.tif")
...
-ANSWER: They all have the same amount of layers, but different scales on the X & Y axis. Terwilliger is also significantly less burned than the other two sites.
...
```{r oreco}
veccy <- st_read("~/Documents/BIO410Stuff/5-OR-fires/OR-ecoregions/Ecoregions_OregonConservationStrategy.shp")
```
...
?st_transform
class(veccy)
veccy <- st_transform(veccy, crs(holidayfarm_rast))
crs(veccy, proj = TRUE) == crs(holidayfarm_rast, proj = TRUE)
```
...
```{r plot-hist}
fire_severe <- c(terwilliger_df$SoilBurnSe, beachiecreek_df$Layer_1, holidayfarm_df$Layer_1)
fire_severe
ggplot(data = as.data.frame(fire_severe), aes(x = as.factor(fire_severe), y = (..count..)/sum(..count..))) +
  scale_fill_brewer(palette = "Spectral", direction=-1) +
  geom_bar() +
  ggtitle("Fire severity count") +
  xlab("Fire severity (1= low, 4= high)") +
  ylab("Proportion")
  


#man why wont it fill :(
```
...
ANSWER: This tells us that intense fires and unburned areas are quite rare, and low and moderate fires are by far the bulk of the data. This relates to the reading because it talks about how  low-medium intensity fires are very common and a natural part of the forest ecosystem, and our data shows that.
